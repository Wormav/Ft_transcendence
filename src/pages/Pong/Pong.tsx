...
export default function Pong() {
	...
	const gameFieldRef = useRef<GameField>({
		width: 0,
		height: 0,
		paddleXPos: 0,
	});
	...
	const [tournamentMatchSettings, setTournamentMatchSettings] =
		useState<TournamentMatchSettings>({
			matchId: null,
		...
		const updateGame = () => {
			...
			if (!editViewMode && ballRef.current) {
				...
				if (particleSystemRef.current) {
					...
					particleSystemRef.current.minLifeTime = Math.max(
						0.1,
						0.3 - speed * 0.5,
					);
					particleSystemRef.current.maxLifeTime = Math.max(
						0.2,
						0.6 - speed * 0.5,
					);
				}
	...
	const resetBall = useCallback(() => {
		...
		const dirZ =
			(Math.random() - 0.5) * speedFactor * (window.innerWidth < 600 ? 0.7 : 1);
	...
	const updatePaddlePositions = useCallback(() => {
		...
		const clampPosition = (pos: number) =>
			Math.max(-paddleLimit, Math.min(pos, paddleLimit));
		paddleLeftRef.current.position.z = clampPosition(
			paddleLeftRef.current.position.z,
		);
		paddleRightRef.current.position.z = clampPosition(
			paddleRightRef.current.position.z,
		);
	}, [editViewMode]);
	...
	const createGameElements = useCallback(
		(scene: BABYLON.Scene) => {
			...
			const paddleMaterial = createMaterial(
				...
				initialPaddleColor,
				new BABYLON.Color3(
					initialPaddleColor.r * 0.5,
					initialPaddleColor.g * 0.5,
					initialPaddleColor.b * 0.5,
				),
			);

			const userPaddleColor = getPaddleColor(
				getPaddleColorFromHex(color_items),
			);
			paddleMaterial.diffuseColor = userPaddleColor;
			paddleMaterial.emissiveColor = new BABYLON.Color3(
				userPaddleColor.r * 0.5,
				userPaddleColor.g * 0.5,
				userPaddleColor.b * 0.5,
			);
	...
	const handleSettingsChange = (newSettings: GameSettings) => {
		...
		if (paddleLeftRef.current && paddleRightRef.current) {
			...
			const paddleMaterial = paddleLeftRef.current
				.material as BABYLON.StandardMaterial;
			const paddleMaterial2 = paddleRightRef.current
				.material as BABYLON.StandardMaterial;
			paddleMaterial.diffuseColor = paddleColor;
			paddleMaterial2.diffuseColor = paddleColor;
		}
		if (sceneRef.current) {
			const ground = sceneRef.current.meshes.find(
	...
	const getPaddleColor = (color: string): BABYLON.Color3 => {
		switch (color) {
			...
			case "purple":
				return new BABYLON.Color3(0.8, 0.2, 0.8);
			default:
	...
	const getPlateauColor = (color: string): BABYLON.Color3 => {
		switch (color) {
			case "blue":
				return new BABYLON.Color3(0.1, 0.1, 0.3);
			case "red":
				return new BABYLON.Color3(0.3, 0.1, 0.1);
			default:
	...
	const updateVisualElements = () => {
		if (paddleLeftRef.current && paddleRightRef.current) {
			try {
				...
				const paddleLeftMaterial = paddleLeftRef.current
					.material as BABYLON.StandardMaterial;
				const paddleRightMaterial = paddleRightRef.current
					.material as BABYLON.StandardMaterial;

				if (paddleLeftMaterial && paddleLeftMaterial.diffuseColor) {
					paddleLeftMaterial.diffuseColor = paddleColor;
					paddleLeftMaterial.emissiveColor = new BABYLON.Color3(
						paddleColor.r * 0.5,
						paddleColor.g * 0.5,
						paddleColor.b * 0.5,
					);
				...
				if (paddleRightMaterial && paddleRightMaterial.diffuseColor) {
					...
					paddleRightMaterial.emissiveColor = new BABYLON.Color3(
						paddleColor.r * 0.5,
						paddleColor.g * 0.5,
						paddleColor.b * 0.5,
					);
			...
			try {
				...
				if (ground && ground.material) {
					...
					if (groundMaterial && groundMaterial.diffuseColor) {
						groundMaterial.diffuseColor = getPlateauColor(
							getPlateauColorFromHex(color_bg),
						);
					}
				}
			} catch (error) {
	...
	const updateBallSpeed = (speed: string) => {
		...
		if (ballDirectionRef.current) {
			...
			BALL_SPEED_INCREASE =
				speed === "turbo" ? 1.08 : speed === "fast" ? 1.06 : 1.05;
		}
	};
	...
	const handleLeftPaddleDown = useCallback(() => {
		if (paddleLeftRef.current && !editViewMode) {
			...
			paddleLeftRef.current.position.z = Math.min(
				paddleLimit,
				paddleLeftRef.current.position.z,
			);
		}
	...
	const handleRightPaddleDown = useCallback(() => {
		if (paddleRightRef.current && !editViewMode) {
			...
			paddleRightRef.current.position.z = Math.min(
				paddleLimit,
				paddleRightRef.current.position.z,
			);
		}
